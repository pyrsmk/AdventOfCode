...
(0..99).each do |noun|
  (0..99).each do |verb|
    ...
    program[1] = Instruction.new(noun)
    program[2] = Instruction.new(verb)
    if Computer.new(program).process[0].to_i == 19690720
      puts "---".colorize(:magenta)
      puts (100 * noun + verb).to_s.colorize(:magenta)
      puts "---".colorize(:magenta)
      exit
    end
  end
...
raise "Relevant noun and verb not found"
